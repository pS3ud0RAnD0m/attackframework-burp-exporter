...
public final class BodyParameterTruncationLog {
    ...
    public static void flushStartupSummary() {
        ...
        ParameterIntegrityDetailReporter.report(
                "startup_backlog",
                "body_params_truncated",
                docCount,
                urlDroppedTotals,
                Map.of("dropped_body_params", droppedTotal(urlDroppedTotals)),
                "request.parameters may omit BODY rows beyond the cap; request.body remains complete",
    ...
    private static void recordLive(String url, int droppedBodyParams) {
        if (LIVE_LOGGED_URLS.add(url)) {
            ParameterIntegrityDetailReporter.report(
                    ...
                    1,
                    Map.of(url, 1),
                    Map.of("dropped_body_params", droppedBodyParams),
                    "request.parameters may omit BODY rows beyond the cap; request.body remains complete",
                    ParameterIntegrityDetailReporter.UrlListPolicy.FULL);
            if (liveUniqueUrlLogsEmitted < LIVE_UNIQUE_URL_LOG_CAP) {
                liveUniqueUrlLogsEmitted++;
                Logger.logDebug(formatLiveLine(droppedBodyParams));
                return;
            }
    ...
    private static String formatOverflowSummary(long additionalUniqueUrls) {
        return "[ParameterIntegrity] BODY parameters truncated for "
                ...
                + ParameterIntegrityDetailReporter.detailPointer("body_params_truncated")
                + ".";
    }

    private static String formatLiveLine(int droppedBodyParams) {
    ...
    private static String formatStartupSummaryLocked() {
        ...
        sb.append("[StartupExport] BODY parameters truncated for ")
                ...
                .append(totalDropped);
        sb.append("; ")
                .append(ParameterIntegrityDetailReporter.detailPointer("body_params_truncated"));
        sb.append('.');
        return sb.toString();
    }
