...
**Log file:** When the extension runs inside Burp, it also writes logs to a file so you can inspect them if the UI freezes: `%USERPROFILE%\.attackframework-burp-exporter\burp-exporter.log` (Windows) or `~/.attackframework-burp-exporter/burp-exporter.log` (Linux/macOS).
